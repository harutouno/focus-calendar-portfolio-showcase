...
現在実装済みの部分（カレンダー・共有・フォーカスタイマー・記録集計・画像添付・RLS・テスト）は、
「予定を管理すること」よりも「計画（予定）と実績（実際にやったこと）を比較し、時間の使い方の
改善につなげること」の基盤として作っています。

以下は構想・検討段階であり、**本リポジトリのコード・テストには含まれていません**。
実装していないものを実装済みのように書かないため、この節でだけ扱います。

- **予実比較**: 集中セッションの計画時間（`FocusSessionRecord.plannedDurationMs`）と
  実際の集中時間（`creditedFocusSeconds`）は記録側に既にありますが、現在は個別セッションの
  記録に留まります。期間をまたいで計画と実績を比較・可視化する機能は未着手です
- **Task（いつやるか未定の作業）**: 現在のカレンダーは「いつやるか決まった予定」だけを扱います。
  日付を決めずに「やること」を保持し、後からカレンダーへ配置する仕組みは、既存の `FocusTask`
  （集中タスク）とは別の概念として設計を検討しています
- **週次レビュー**: 日 / 週 / 月の記録集計を、振り返りの導線として機能させること

いずれも思想の段階であり、設計・実装の途中経過を「完成」と称することはしません。
